...
class PaymentApi {
  PaymentApi._sharedInstance();
  static final _shared = PaymentApi._sharedInstance();
  factory PaymentApi() => _shared;

  Future<List<Payment>> getAllPayment() async {
    List<Payment> payments = [];
    final uri = Uri.parse("$baseUrl/$paymentsRoute");
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode <= 299) {
        ...
        payments = jsonData.map((e) => Payment.fromJson(e)).toList();
      }
  ...
  Future<Payment?> getPaymentById(int paymentId) async {
    ...
    final uri = Uri.parse("$baseUrl/$paymentsRoute/$paymentId");
    try {
      final response = await http.get(
        uri,
        headers: headers,
      ...
      if (response.statusCode >= 200 && response.statusCode <= 299) {
        ...
        payment = Payment.fromJson(jsonData);
      }
    } on http.ClientException {
      throw ApiClientException();
    } catch (e) {
  ...
  Future<List<Payment>> getPaymentBySessionId(int sessionId) async {
    List<Payment> payments = [];
    final uri =
        Uri.parse("$baseUrl/$paymentsRoute?$sessionIdColumn=$sessionId");
    try {
      final response = await http.get(
        uri,
        headers: headers,
  ...
  Future<http.Response> deletePayment(int paymentId) async {
    final uri = Uri.parse("$baseUrl/$paymentsRoute/$paymentId");
    try {
